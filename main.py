...
async def on_message(message):
    ...
    if message.content.startswith(".add"):
        ...
        if roblox_id in existing_ids:
            ...
            id_map[roblox_id] = f"{roblox_id} - {get_roblox_usernames([roblox_id]).get(roblox_id, 'Unknown User')} ({user_tag})"
            embed = discord.Embed(
                title="✅ Successfully Added!",
                description=f"**{roblox_id}** - **{get_roblox_usernames([roblox_id]).get(roblox_id, 'Unknown User')}** has been added to the whitelist for {user_tag}.",
        ...
        if old_id not in id_map or id_map[old_id].split(' ')[1] != f"({user_tag})":
        ...
        usernames = get_roblox_usernames([new_id])
        username = usernames.get(new_id, "Unknown User")
        id_map[new_id] = f"{new_id} - {username} ({user_tag})"
